    ...
    config = function()
      ...
      local NeoTest = {
        ...
        },
        {
          ...
          init = function(self)
            self.adapter = vim.split(vim.split(self.adapter_ids[1], ':', { plain = true })[1], 'neotest-', { plain = true })[2]
          end,
      ...
      local FileNameBlock = {
        ...
        init = function(self)
          ...
          if ok then
            self.icon, self.icon_color = devicons.get_icon_color(self.filename, fn.fnamemodify(self.filename, ':e'), { default = true })
          else
            self.icon = ''
      ...
      local Git = {
        init = function(self)
          ...
          self.status.head = u.count_chars(self.status.head) < max and self.status.head or string.format('%s...', string.sub(self.status.head, 1, max))
        end,
        {
          provider = function(self)
      ...
      local DAPBlock = {
        condition = function()
          ...
          return session ~= nil and conditions.is_active() and not conditions.buffer_matches(dap_inactive)
        end,
        provider = function()
      ...
      local StatusLines = {
        ...
        DiagnosticsBlock,
        {
          ...
          hl = { bg = colors.null, fg = colors.dragonRed },
        },
        RulerBlock,
        hl = function()
          if conditions.is_active() then
